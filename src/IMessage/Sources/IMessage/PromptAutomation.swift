...
private let log = Logger(imessageLabel: "prompt-automation")
...
enum PromptAutomation {
    static func confirmUNCPrompt() throws {
        ...
        try retry(withTimeout: 2.5, interval: 0.05) { () throws in
            ...
            guard let lastButton = try window.children().last(where: { (try? $0.role()) == Accessibility.Role.button }) else {
    ...
    static func disableNotificationsForApp(named appName: String) throws -> Bool {
        ...
        return try retry(withTimeout: 3, interval: 0.1) {
            ...
            if #available(macOS 13, *) {
                let settingsView = try window.children().first(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("settingsView not found"))
                let settingsSplitGroupView = try settingsView.children().first(where: { (try? $0.role()) == Accessibility.Role.splitGroup }).orThrow(ErrorMessage("settingsSplitGroupView not found"))

                // right side pane is last
                let settingsPaneView = try settingsSplitGroupView.children().last(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("settingsPaneView not found"))
                let settingsPaneGroupView = try settingsPaneView.children().first(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("settingsPaneGroupView not found"))
                let scrollView = try settingsPaneGroupView.children().first(where: { (try? $0.role()) == Accessibility.Role.scrollArea }).orThrow(ErrorMessage("scrollView not found"))
                ...
                var notificationsScrollView = try settingsPaneGroupView.children().first(where: { (try? $0.role()) == Accessibility.Role.scrollArea }).orThrow(ErrorMessage("notificationsScrollView not found"))
                var allowNotificationsView = try notificationsScrollView.children().first(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("allowNotificationsView not found"))

                if (try? allowNotificationsView.children().last(where: { (try? $0.role()) == Accessibility.Role.staticText })?.value() as? String) != appName {
                    log.debug("Not in \(appName) settings, going back to main notification center settings. Current appName:\((try? allowNotificationsView.children().last(where: { (try? $0.role()) == Accessibility.Role.staticText })?.value() as? String) ?? "nil")")
                    let toolbarView = try window.children().first(where: { (try? $0.role()) == Accessibility.Role.toolbar }).orThrow(ErrorMessage("toolbarView not found"))
                    let toolbarButton = try toolbarView.children().first(where: { (try? $0.role()) == Accessibility.Role.button }).orThrow(ErrorMessage("toolbarButton not found"))
                    try toolbarButton.press()

                    try Self.openNotificationSettingsForApp(appName: appName, scrollView: scrollView)

                    notificationsScrollView = try settingsPaneGroupView.children().first(where: { (try? $0.role()) == Accessibility.Role.scrollArea }).orThrow(ErrorMessage("notificationsScrollView not found"))
                    allowNotificationsView = try notificationsScrollView.children().first(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("allowNotificationsView not found"))
                }

                let notificationsSwitch = try allowNotificationsView.children().first(where: { (try? $0.subrole()) == Accessibility.Subrole.switch }).orThrow(ErrorMessage("switch not found"))
                ...
                let scrollView = try tabView.children().first(where: { (try? $0.role()) == Accessibility.Role.scrollArea }).orThrow(ErrorMessage("scrollView not found"))
                let paneView = try tabView.children().first(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("paneView not found"))
                let tableView = try scrollView.children().first(where: { (try? $0.role()) == Accessibility.Role.table }).orThrow(ErrorMessage("tableView not found"))
                let targetRow = try tableView.children().first(where: {
                    (try? $0.role()) == Accessibility.Role.row &&
                ...
                let notificationsSwitch = try paneView.children().first(where: { (try? $0.subrole()) == Accessibility.Subrole.switch }).orThrow(ErrorMessage("switch not found"))
    ...
    private static func openNotificationSettingsForApp(appName: String, scrollView: Accessibility.Element) throws {
        ...
        let appsListView = try scrollView.children().last(where: { (try? $0.role()) == Accessibility.Role.group }).orThrow(ErrorMessage("appsListView not found"))

        let targetButton = try appsListView.children().first(where: {
            (try? $0.role()) == Accessibility.Role.button &&

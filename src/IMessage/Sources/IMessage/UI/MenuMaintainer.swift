...
private let log = Logger(imessageLabel: "menu-maintainer")
...
final class MenuMaintainer {
    ...
    private static var maintenancePeriod: TimeInterval {
        Defaults.imessage.double(forKey: DefaultsKeys.settingsMenuItemInjectionMaintenancePeriod)
    }
    ...
    private func spawnMaintainer() {
        maintainer = Task {
            ...
            while beganInjectionAttempts.timeIntervalSinceNow > -Self.maintenancePeriod {
                ...
                let ms = 1000 * Defaults.imessage.double(forKey: DefaultsKeys.settingsMenuItemInjectionMaintenanceInterval)
...
private func findSuitableInjectionTarget(in mainMenu: NSMenu) throws(InjectionError) -> NSMenu {
    let defaultTitle = Defaults.imessage.string(forKey: DefaultsKeys.settingsMenuItemInjectionDefinitelyNotReadyMenuItemTitle)
    ...
    let injectionTargetSubstring = Defaults.imessage.string(forKey: DefaultsKeys.settingsMenuItemInjectionTargetSubstring) ?? ""

...
private let log = Logger(imessageLabel: "contacts")
...
final class Contacts {
    ...
    private lazy var shortFormatter: CNContactFormatter? = {
        guard Defaults.imessage.bool(forKey: DefaultsKeys.contactsAttemptFormattingWithShortStyle) else {
    ...
    private lazy var descriptorForShortFormatting: (any CNKeyDescriptor)? = {
        guard Defaults.imessage.bool(forKey: DefaultsKeys.contactsAttemptFormattingWithShortStyle), let short = CNContactFormatterStyle.short else {
            return nil
        }

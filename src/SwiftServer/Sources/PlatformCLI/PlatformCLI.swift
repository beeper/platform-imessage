...
struct PlatformCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "PlatformCLI",
        abstract: "Exercise the native iMessage PlatformAPI.",
        discussion: """
        Bare launch, or the `shell` command, opens the interactive shell.
        Run `PlatformCLI help` for the command list or `PlatformCLI help COMMAND` for command help.
        """
    )
...
private final class InvokeContext {
    ...
    func api() async -> PlatformCLIAPI {
        await runner.api()
    }

    func invoke(_ methodName: String, args: [Any], _ operation: @escaping (PlatformCLIAPI) async throws -> String?) async throws {
...
private final class Runner {
    ...
    private var apiInstance: PlatformCLIAPI?
    ...
    func run() async throws {
        ...
        PlatformCLIBootstrap.bootstrap(
    ...
    func api() async -> PlatformCLIAPI {
        ...
        let created = PlatformCLIAPI(accountID: accountID)
        apiInstance = created
        return created
    }
    ...
    func invoke(
        ...
        methodName: String,
        args: [Any],
        _ operation: @escaping (PlatformCLIAPI) async throws -> String?
    ) async throws {
        let api = await api()
    ...
    private func ensureEventSubscription(_ api: PlatformCLIAPI) async throws {
...
private func printTopLevelHelp() {
    var lines = [
        ...
        "",
        "Usage:",
        "  PlatformCLI [global options]",
        "  PlatformCLI COMMAND [ARGS...]",
        "",
...
private func authorizeAutomation(api: PlatformCLIAPI) async -> Bool {

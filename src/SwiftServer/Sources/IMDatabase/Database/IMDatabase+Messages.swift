...
SELECT c.guid, m.ROWID, m.guid, m.balloon_bundle_id, m.text, m.attributedBody, m.is_from_me, m.is_sent, m.date, m.date_read, m.message_summary_info
FROM message m
...
private extension Message {
    init(row: borrowing Row) throws {
        ...
        self = try Message(
            ...
            text: row[4].optional(String.self).map {
                Sensitive(.messageText, hiding: $0)
            },
            attributedBody: row[5].optional(Data.self).flatMap {
                try Sensitive(.messageAttributedBody, hiding: unarchiveAttributedString(from: $0))
            },
            isFromMe: row[6].looseBool(),
            isSent: row[7].looseBool(),
            date: row[8].imCoreDate(),
            dateRead: row[9].imCoreDate(),
            summaryInfo: row[10].optionalConverting(Data.self).map(Message.SummaryInfo.init(blob:)),
        )
    }
}

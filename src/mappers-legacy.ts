...
import { Message, Attachment, AttachmentType, MessageActionType, MessageBehavior, MessageReaction, TextAttributes, TextEntity } from '@textshq/platform-sdk'
...
import type { MappedAttachmentRow, MappedMessageRow, MappedReactionMessageRow, MessageSummaryInfo } from './types'
...
// Temporary TypeScript source-of-truth mapper. Delete this file once Swift
// mapMessage parity has baked enough that the runtime guard can be removed.
...
function decodeMessageParts(fragments: Fragment[], messageSummaryInfo?: MessageSummaryInfo): MessagePart[] {
  ...
  for (const frag of fragments) {
    ...
    if (partNumber != null) {
      ...
      // "rp".
      if (partNumber !== (lastSeenPart ?? -1) + 1 && deletedParts?.includes(partNumber - 1)) {
...
export function mapMessageLegacy(msgRow: MappedMessageRow, attachmentRows: MappedAttachmentRow[] = [], reactionRows: MappedReactionMessageRow[], currentUserID: string, accountID: string): BeeperMessage[] {
  ...
  const appleDateFromRow = (date: AppleDate | null | undefined, fallback: AppleDate | number | null | undefined): AppleDate | undefined => {
    const value = date ?? fallback
    return value == null ? undefined : String(value) as AppleDate
  }
  const messageDateString = appleDateFromRow(msgRow.dateString, msgRow.date)
  const dateReadString = appleDateFromRow(msgRow.dateReadString, msgRow.date_read)
  const dateEditedString = appleDateFromRow(msgRow.dateEditedString, msgRow.date_edited)
  const dateRetractedString = appleDateFromRow(msgRow.dateRetractedString, msgRow.date_retracted)

  if (msgRow.schedule_type) return []

  const partialMessage: BeeperMessage = {
    id: msgRow.guid,
    cursor: messageDateString,

    // TODO(skip): probably throw (or drop) instead of `0`.
    timestamp: messageDateString ? regularlizeAppleDate(messageDateString) ?? new Date(0) : new Date(0),
    ...
    sortKey: messageDateString ? appleDateToMillisSinceEpoch(messageDateString) ?? 0 : 0,
    ...
    seen: !isGroup && dateReadString ? regularlizeAppleDate(dateReadString) : undefined,
  ...
  if (dateStringIsTruthy(dateRetractedString) || msgRow.was_detonated) partialMessage.isDeleted = true
  if (msgRow.is_read) {
  ...
  // somewhat misleading.
  if (!unsendDataPresent && dateEditedString && dateStringIsTruthy(dateEditedString)) {
    partialMessage.editedTimestamp = regularlizeAppleDate(dateEditedString) ?? new Date(0)
  }

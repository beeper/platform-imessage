...
- concurrency
  - [x] review for races, `PlatformAPI.messagesController` is mutated without isolation — mutation now lives in the `MessagesControllerCoordinator` actor; `lastOpenedThreadID`/activity state verified lane-confined (documented in MessagesController.swift); `SpacesWindowCoordinator`'s observer-callback races fixed with a `Protected` state box
  - [x] adopt strict-concurrency checking — `-strict-concurrency=targeted` is on the `IMessage` target and all 33 concurrency warnings are cleared with real isolation: `PlatformAPI` and both window coordinators are compiler-checked `Sendable`, `HideDebouncer` is `@MainActor`, lane generics require `T: Sendable`, `Pasteboard.withRestoration` restoration is structured (no longer lost on cancellation), PlatformSDK `Message`/`Thread` are `@unchecked Sendable` only for their `Any`-typed JSON fields. Next step: escalate to `-strict-concurrency=complete` (or Swift 6 mode) once the remaining targets adopt it.
